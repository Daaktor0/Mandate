...
TYPESCRIPT_OUTPUT = (
    ROOT / "packages" / "shared-schemas" / "typescript" / "generated.ts"
)
...
def python_field_type(
    schema: dict[str, Any], name: str, context: PyContext
) -> str:
    if "const" in schema:
        value = schema["const"]
    ...
    if enum_values:
        ...
        if enum_name not in context.emitted_names:
            ...
            body = "\n".join(
                f"    {enum_member(str(value))} = {py_string(str(value))}"
...
def emit_python_model(
    model_name: str, schema: dict[str, Any], context: PyContext
) -> None:
    ...
    for property_name, property_schema in properties.items():
        ...
        fields.append(
            (
                ...
                type_name,
                python_field_expression(
                    property_name, property_schema, property_name in required
                ),
            )
        )
...
def zod_expression(
    schema: dict[str, Any], name: str, context: TsContext
) -> str:
    if "const" in schema:
...
def emit_typescript_schema(
    schema_name: str, schema: dict[str, Any], context: TsContext
) -> None:
