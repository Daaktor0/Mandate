...
as $$
  select coalesce(cardinality(p_values), 0) = (
    select count(distinct value)::integer
      from unnest(coalesce(p_values, '{}'::uuid[])) as item(value)
  );
$$;

revoke all on function private.uuid_array_is_unique(uuid[])
  from public, anon, authenticated;
grant execute on function private.uuid_array_is_unique(uuid[])
  to service_role;

alter table public.report_requests
  add constraint report_requests_related_entity_scope check (
    cardinality(related_entity_ids) <= 2
    and private.uuid_array_is_unique(related_entity_ids)
    and (
      confirmed_entity_id is null
      or not confirmed_entity_id = any(related_entity_ids)
    )
  );

create table private.entity_confirmation_commands (
  ...
  primary key (user_id, report_request_id, idempotency_key)
);

comment on table private.entity_confirmation_commands is
  'Private replay ledger for tenant-scoped entity confirmation decisions.';

revoke all on table private.entity_confirmation_commands
  from public, anon, authenticated;
grant select, insert, update, delete on table private.entity_confirmation_commands
  to service_role;
