...
| INTAKE-04 | No confidential free-form descriptions or documents | C1, C2 | strict generated intake schema; body cap; mandatory ack in route/RPC; privacy-allowlisted `SearchRequest` and URL-only `PageFetchRequest`; answer screening pending | AT-INTAKE-04 (route + DB ack + provider extra-field rejection), E2E-03 (pending) | 1/3 | In progress |
...
| RUN-06 | Fetched content treated as untrusted | C8, C9, C6 | Exa results are discovery metadata only; robots-aware `PageFetcher` delegates to SafeFetcher, strips active/hidden markup, flags raw-source injection patterns, blocks access-control indicators and returns `evidence_admitted=false`; model untrusted envelopes pending | AT-RUN-06 (search/PageFetcher payload, extraction, non-bypass and non-admission foundations; gateway pending), SEC-04 and ER-10 malicious-page acceptance green | 2 | In progress |
| RUN-07 | Per-report search/page/token/time/retry budgets | C6, C7, C8 | bounded Exa request/result/call/time/body limits plus PageFetcher robots/page size/delay/redirect constraints implemented; report-level budget profile and orchestration pending (QUEUE §8) | AT-RUN-07 (provider bounds/retry foundations) | 2 | Specified |
...
| NFR-05 | Every external cost attributable to a report | C7, C8, C15 | bounded company-data call/query counts, Exa call count/reported `costDollars.total` and PageFetcher call count returned for later `provider_cost_events`; report attribution/admin view pending | AT-NFR-05 (provider/candidate/search/page-fetch foundations; persistence pending) | 2 | In progress |
