...
| INTAKE-04 | No confidential free-form descriptions or documents | C1, C2 | strict generated intake schema; body cap; mandatory ack in route/RPC; privacy-allowlisted `SearchRequest`, URL-only `PageFetchRequest` and identifier-only `ConfirmedCorporateFilingCommand`; answer screening pending | AT-INTAKE-04 (route + DB ack + provider/command extra-field rejection), E2E-03 (pending) | 1/3 | In progress |
...
| RUN-03 | Bounded business/industry/competitor/corporate/regulatory/public-risk/synthesis tasks | C6 | pipeline stages 2–10; typed stages 2–7 run search → PageFetcher → admission → ModelGateway and emit `AgentFinding` | AT-RUN-03; `services/worker/tests/test_research_stages.py`; `test_golden_fixtures.py`; GC-01..15 corpus | 2 | In progress |
| RUN-04 | Evidence stored separately from prose | C4 | `evidence`, `claims` vs `report_versions`; bounded evidence excerpts and normalised claim triples are separate persistence surfaces; explicit `UntrustedEvidenceCandidate` → shared `Evidence` admission | AT-RUN-04, E2E-02; `services/worker/tests/test_evidence_capture.py`; `test_golden_fixtures.py`; migration/pgTAP foundation | 2 | In progress |
...
| RUN-06 | Fetched content treated as untrusted | C8, C9, C6 | Exa results are discovery metadata only; robots-aware `PageFetcher` delegates to SafeFetcher, strips active/hidden markup, flags raw-source injection patterns, blocks access-control indicators and returns `evidence_admitted=false`; explicit evidence admission preserves suspicion flags and fails closed on unknown source tiers; confirmation-gated filing acquisition produces only SHA-256-addressed `pending_malware_scan`, `parse_allowed=false` references; the file-safety boundary (quarantine integrity → malware scan → archive limits → attested sandbox parse) is the sole route from a quarantined binary to text and returns it `untrusted=true`, `evidence_admitted=false` with live parsing fail-closed; ModelGateway now wraps admitted excerpts in bounded untrusted envelopes with deterministic injection suspicion flags | AT-RUN-06 (`test_evidence_capture.py`, `test_prompt_safety_budgets.py`, gateway payload assertions), SEC-04 and ER-10 malicious-page acceptance green, SEC-05 hostile-file foundations green; GC-15 | 2 | In progress |
| RUN-07 | Per-report search/page/token/time/retry budgets | C6, C7, C8 | bounded Exa request/result/call/time/body limits plus PageFetcher robots/page size/delay/redirect constraints and filing size/count bounds; `BudgetProfile.mvp_standard` supplies the QUEUE §8 job caps, six research-stage slices and deterministic stop decisions; `BudgetLedger` enforces counters before provider/model work | AT-RUN-07 (`test_prompt_safety_budgets.py` and provider-bound suites) | 2 | In progress |
...
| REPORT-06 | Distinguish fact/company claim/third-party/inference/conflict/unavailable | C6, C4 | `claims.claim_type`; stage-8 verifier statuses, contradiction records and coverage report; source-tiered `Evidence` metadata and preserved prompt-injection flag | `test_verifier.py`; `test_golden_fixtures.py`; AT-REPORT-06, QG; `test_evidence_capture.py`; claim enum and provenance migration foundation | 2/4 | In progress |
...
| REPORT-08 | Dynamic research: last 3 FYs + current, or latest available | C6 | stage 2–7 draft validation requires current/recent periods inside the bounded plan window; `claims.freshness` and `claims.period` remain separate | AT-REPORT-08; `services/worker/tests/test_research_stages.py`; persistence foundation | 2 | In progress |
| REPORT-09 | Historical facts may extend to incorporation | C6 | stage 2–7 accepts dated historical periods only at or after the bounded incorporation floor; `claims.freshness` and `claims.period` remain separate | AT-REPORT-09; `services/worker/tests/test_research_stages.py`; persistence foundation | 2 | In progress |
| REPORT-10 | Public-risk only where entity matching reliable | C6 (public-risk agent) | strong-identifier matching (AGENT-PROMPT §5) | AT-REPORT-10, GC-12; `test_golden_fixtures.py` | 2 | In progress |
...
| NFR-05 | Every external cost attributable to a report | C7, C8, C15 | bounded company-data call/query counts, Exa call count/reported `costDollars.total`, PageFetcher call count and filing-provider call count returned for `provider_cost_events`; ModelGateway cost-cap enforcement and per-call INR cost capture in `agent_runs`; report attribution/admin view pending | AT-NFR-05 (provider/candidate/search/page-fetch/filing/model-gateway foundations; migration/pgTAP foundation) | 2 | In progress |
...
| NFR-09 | Store model IDs, prompt versions, parameters, evidence | C7, C4 | sanitised agent-run record + prompt/routing/model capture in code and `agent_runs`; `claims`/`evidence` provenance tables now present; durable sink integration pending | AT-NFR-09 (gateway unit foundations; migration/pgTAP foundation) | 2 | In progress |
