...
| NFR-02 | Tenant isolation at database layer | C4 | RLS on all tables (ERD §4) | AT-NFR-02, SEC-01 | 0+ | In progress |
| NFR-03 | Containerised, Hostinger-independent worker | C6 | `services/worker/Dockerfile`; `infra/compose/local.yml`; adapters; no host coupling | AT-NFR-03 (structural + live portability/sandbox check) | 0 | Verified |
